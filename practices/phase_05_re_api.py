...
def e3_challenge_1():
    """re.sub 替换：将所有邮箱替换为 [REDACTED]"""
    pattern = r""  # 提示：\S+@\S+\.\S+
    return test(
        pattern,
        [
            ("单邮箱",   "email: alice@test.com",       "email: [REDACTED]"),
            ("多邮箱",   "a@x.com and b@y.com",         "[REDACTED] and [REDACTED]"),
...
def e_mixed_1():
    ...
    pattern = r""  # 提示：r"^.*(WARNING|ERROR):.*$" + re.MULTILINE
    return test(
        pattern,
        [
            ("多行日志", log, True),
